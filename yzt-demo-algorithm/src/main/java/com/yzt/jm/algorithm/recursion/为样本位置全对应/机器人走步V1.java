...
public class 机器人走步V1 {
     ...
     * @return 多少种走法
     */
    public static int go(int cur, int rest, int to, int n) {
        if (rest == 0) {
            return cur == to ? 1 : 0;
        }
        if (cur == 0) {
            return go(2, rest - 1, to, n);
        } else if (cur == n) {
            return go(n - 1, rest - 1, to, n);
        } else {
            return go(cur - 1, rest - 1, to, n) + go(cur + 1, rest - 1, to, n);
        }
    }
